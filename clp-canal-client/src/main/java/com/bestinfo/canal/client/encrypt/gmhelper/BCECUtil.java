...
public class BCECUtil {
	...
	public static ECPublicKeyParameters createECPublicKeyParameters(byte[] bytes, ECCurve curve,
			ECDomainParameters domainParameters) {
		final byte uncompressedFlag = 0x04;
		byte[] encodedPubKey = new byte[1 + bytes.length];
		encodedPubKey[0] = uncompressedFlag;
		System.arraycopy(bytes, 0, encodedPubKey, 1, bytes.length);
